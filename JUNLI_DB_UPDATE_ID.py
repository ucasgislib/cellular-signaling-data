...
class UPDATE():
    ...
    def sel_data(self,TableName):
        SQL0 = "SELECT imsi,home_cid,home_lac,work_cid,work_lac,traffic_type,file_num FROM bj_userinfos_final_noid;"
        ...
        for num, row in enumerate(res):
            ...
            home_sel = "SELECT  imsi,home_cid,home_lac,work_cid,work_lac,traffic_type,file_num FROM bj_usershw_0504 where imsi=%d "%(imsi)
            self.cur.execute(home_sel)
            home_res = self.cur.fetchall()
            if len(home_res)>0:
                imsi,home_cid,home_lac,work_cid,work_lac,traffic_type,file_num = home_res[0]
                SQL = '''INSERT INTO %s (imsi,home_cid,home_lac,work_cid,work_lac,traffic_type,file_num) 
               VALUES(%d,%d,%d,%d,%d,%d,%d) ;'''%(TableName,imsi,home_cid,home_lac,work_cid,work_lac,traffic_type,file_num)
                self.cur.execute(SQL)
                cq+=1
            else:
                SQL = '''INSERT INTO %s (imsi,home_cid,home_lac,work_cid,work_lac,traffic_type,file_num) 
