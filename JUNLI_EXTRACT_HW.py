...
class BATCH():
    ...
    def run(self):
        ...
        fo = open("orig_tongqin.csv",'w')
        fu =open("userinfos_tongqin.csv",'w')
        
        cp,cq,ct = 0,0,0
        for data_imsi in que_res:
            imsi,h_lac,h_cid,w_lac,w_cid,u_typ = int(data_imsi['uid']),int(data_imsi['h_lac']),int(data_imsi['h_cid']),int(data_imsi['w_lac']),int(data_imsi['w_cid']),int(data_imsi['u_typ'])
            try:
                file_num = imsi_filenum_dict[imsi]
            except:
                cq+=1
                continue
            # print imsi,h_lac,h_cid,w_lac,w_cid,u_typ,file_num
            # try:
            #     hx,hy,wx,wy = self.cell_dict[(h_lac,h_cid)]['x'],self.cell_dict[(h_lac,h_cid)]['y'],self.cell_dict[(w_lac,w_cid)]['x'],self.cell_dict[(w_lac,w_cid)]['y']
            #     dist = self.cal_Eu_dist(hx,hy,wx,wy)/1000.00
            # except:
            #     rec = [str(r) for r in [imsi,h_lac,h_cid,w_lac,w_cid,u_typ,500]]
            #     fu.write(",".join(rec)+"\n")
            #     continue
            ...
            ORIG = True
            if ORIG:
                f=open("../bj02_imsitim/part-00%03d"%file_num,'r')
