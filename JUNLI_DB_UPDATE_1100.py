...
class UPDATE():
    ...
    def sel_data(self,TableName):
        SQL0 = "SELECT count(*) FROM bj_userinfos_final where traffic_type = 1104;"
        self.cur.execute(SQL0)
        print self.cur.fetchall()
        SQL0 = "SELECT imsi,home_cid,home_lac,work_cid,work_lac,traffic_type,file_num,home_community_id,home_district_id,work_community_id,work_district_id FROM bj_userinfos_final where traffic_type = 1104;"
        self.cur.execute(SQL0)
        ...
        for num, row in enumerate(res):
            ...
            try:
                home_sel = "SELECT x,y FROM bj_cellinfos_id where lac=%d and cid = %d"%(home_lac,home_cid)
                self.cur.execute(home_sel)
                home_x,home_y = self.cur.fetchone()
                work_sel = "SELECT x,y FROM bj_cellinfos_id where lac=%d and cid = %d"%(work_lac,work_cid)
                ...
                # print SQL
                SQL = '''UPDATE bj_userinfos_final SET traffic_type = %d WHERE imsi = %d ;'''%(traffic_type,imsi)
                self.cur.execute(SQL)
           #      # print "----------"
